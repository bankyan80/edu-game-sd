...
export default function CariKata() {
  const game = getGameBySlug("cari-kata")!;
  const allQuestions = useMemo(() => getQuestions("cari-kata"), []);
      ...
      {(props) => {
        ...
        return (
          <div className="max-w-2xl mx-auto">
            ...
            <div className="bg-gradient-to-br from-indigo-300 to-violet-400 rounded-2xl p-6 shadow-lg mb-6">
              <div className="flex items-center gap-2 mb-3"><span className="text-2xl">🔍</span><span className="text-white/80 text-sm font-medium">Cari Kata</span></div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {q.options?.map((opt, i) => (
                <motion.button key={i} whileHover={{scale:1.03}} whileTap={{scale:0.97}} onClick={() => handleAnswer(opt)} disabled={isAnswered}
                  className={`p-4 rounded-xl font-bold text-lg transition-all ${isAnswered && opt === q.answer ? "bg-green-500 text-white shadow-lg" : isAnswered && opt !== q.answer ? "bg-red-100 text-red-400" : "bg-white text-indigo-700 hover:bg-indigo-50 border-2 border-indigo-200"}`}>
                  {opt}
                </motion.button>
              ))}
            </div>
