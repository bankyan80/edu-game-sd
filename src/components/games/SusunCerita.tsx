        ...
        return (
          <div className="max-w-2xl mx-auto">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-lg mb-4">
              <p className="font-bold text-gray-800 mb-4">Susun urutan cerita yang benar:</p>
              <div className="min-h-[80px] bg-gray-50 rounded-xl p-3 mb-4">
                {selected.length === 0 ? <span className="text-gray-400 text-sm">Klik langkah di bawah</span> :
                  selected.map((idx, pos) => (
                    <motion.button key={pos} initial={{scale:0}} animate={{scale:1}} onClick={() => removeItem(pos)}
                      className="inline-block px-3 py-2 bg-amber-500 text-white rounded-lg font-bold text-sm m-1">
                      {pos + 1}. {items[idx]} ×
                    </motion.button>
                  ))
                }
            ...
            <div className="flex flex-wrap gap-2 mb-4 justify-center">
              {items.map((item, i) => (
                <motion.button key={i} whileHover={{scale:1.05}} whileTap={{scale:0.95}} onClick={() => addItem(i)}
                  ...
                  className={`px-4 py-2 rounded-xl font-medium text-sm transition-all ${selected.includes(i) ? "bg-gray-200 text-gray-400" : "bg-amber-100 text-amber-700 hover:bg-amber-200"}`}>
                  {item}
                </motion.button>
              ))}
            </div>
            <button onClick={handleSubmit} disabled={isAnswered || selected.length !== items.length}
              className="w-full py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl font-bold disabled:opacity-50 transition">
              Jawab
            </button>
