      ...
      {(props) => {
        ...
        return (
          <div className="max-w-2xl mx-auto h-full flex flex-col">
            ...
            <div className="bg-gradient-to-br from-orange-300 to-red-400 rounded-2xl p-3 shadow-lg mb-2 text-center">
            ...
            <div className="space-y-1.5">
              {q.options?.map((opt, i) => (
                <motion.button key={i} whileHover={{x:4}} whileTap={{scale:0.98}} onClick={() => handleAnswer(opt)} disabled={isAnswered}
                  className={`w-full p-2.5 rounded-xl font-bold text-left transition-all flex items-center gap-3 ${isAnswered && opt === q.answer ? "bg-green-500 text-white shadow-lg" : isAnswered && opt !== q.answer ? "bg-red-100 text-red-400" : "bg-white text-gray-700 hover:bg-orange-50 border-2 border-gray-200 hover:border-orange-300"}`}>
