      ...
      {(props) => {
        ...
        return (
          <div className="max-w-2xl mx-auto">
            ...
            <div className="bg-gradient-to-br from-blue-300 to-blue-500 rounded-2xl p-6 shadow-lg mb-6">
              <div className="flex items-center gap-2 mb-3"><span className="text-2xl">🌐</span><span className="text-white/80 text-sm font-medium">Quiz Bahasa Inggris</span></div>
            ...
            <div className="space-y-3">
              {q.options?.map((opt, i) => (
                <motion.button key={i} whileHover={{ x: 4 }} whileTap={{ scale: 0.98 }} onClick={() => handleAnswer(opt)} disabled={isAnswered}
                  className={`w-full p-4 rounded-xl font-bold text-left transition-all flex items-center gap-3 ${isAnswered && opt === q.answer ? "bg-green-500 text-white shadow-lg" : isAnswered && opt !== q.answer ? "bg-red-100 text-red-400" : "bg-white text-gray-700 hover:bg-blue-50 border-2 border-gray-200 hover:border-blue-300"}`}>
