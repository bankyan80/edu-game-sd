      ...
      {(props) => {
        ...
        return (
          <div className="max-w-2xl mx-auto h-full flex flex-col">
            ...
            <div className="bg-gradient-to-br from-indigo-300 to-purple-400 rounded-2xl p-3 shadow-lg mb-2 text-center">
            ...
            <div className="grid grid-cols-2 gap-2">
              {q.options?.map((opt, i) => (
                <motion.button key={i} whileHover={{scale:1.03}} whileTap={{scale:0.97}} onClick={() => handleAnswer(opt)} disabled={isAnswered}
                  className={`p-2.5 rounded-xl font-bold text-sm transition-all ${isAnswered && opt === q.answer ? "bg-green-500 text-white shadow-lg" : isAnswered && opt !== q.answer ? "bg-red-100 text-red-400" : "bg-white text-purple-700 hover:bg-purple-50 border-2 border-purple-200"}`}>
                  {opt}
                </motion.button>
              ))}
            </div>
