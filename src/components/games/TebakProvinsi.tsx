            ...
            <div className="flex flex-col gap-2 flex-1 min-h-0">
              {q.options?.map((opt, i) => (
                <motion.button key={i} whileHover={{x:4}} whileTap={{scale:0.98}} onClick={() => handleAnswer(opt)} disabled={isAnswered}
                  className={`w-full flex-1 p-3 rounded-xl font-bold text-left transition-all flex items-center gap-3 ${isAnswered && opt === q.answer ? "bg-green-500 text-white shadow-lg" : isAnswered && opt !== q.answer ? "bg-red-100 text-red-400" : "bg-white text-gray-700 hover:bg-pink-50 border-2 border-gray-200 hover:border-pink-300"}`}>
