...
function generateQuestions(): Q[] {
  ...
  return shuffled.map((p) => {
    const wrong = pakaianAdatData
      .filter((x) => x.provinsi !== p.provinsi)
      .sort(() => Math.random() - 0.5)
      .slice(0, 3)
      .map((x) => x.provinsi);
    const options = [p.provinsi, ...wrong].sort(() => Math.random() - 0.5);
    return {
      question: `${p.emoji} ${p.nama}`,
      answer: p.provinsi,
      options,
    };
  });
...
export default function CocokkanPakaian() {
  const game = getGameBySlug("cocokkan-pakaian")!;
      ...
      {(props) => {
        ...
        return (
          <div className="max-w-2xl mx-auto h-full flex flex-col">
            ...
            <div className="bg-gradient-to-br from-rose-300 to-pink-400 rounded-2xl p-4 shadow-lg mb-3 text-center shrink-0">
              <p className="text-sm text-white/80 mb-1">Pakaian adat ini berasal dari provinsi mana?</p>
