...
export default function QuizBahasaIndonesia() {
  const game = getGameBySlug("quiz-bahasa-indonesia")!;
  const allQuestions = useMemo(() => getQuestions("quiz-bahasa-indonesia"), []);
      ...
      {(props) => {
        ...
        return (
          <div className="max-w-2xl mx-auto h-full flex flex-col">
            ...
            <div className="bg-gradient-to-br from-red-300 to-red-500 rounded-2xl p-4 shadow-lg mb-3 shrink-0">
              <div className="flex items-center gap-2 mb-1"><span className="text-2xl">📖</span><span className="text-white/80 text-sm font-medium">Quiz Bahasa Indonesia</span></div>
            ...
            <div className="flex flex-col gap-2 flex-1 min-h-0">
              {q.options?.map((opt, i) => (
                <motion.button key={i} whileHover={{ x: 4 }} whileTap={{ scale: 0.98 }} onClick={() => handleAnswer(opt)} disabled={isAnswered}
                  className={`w-full flex-1 p-3 rounded-xl font-bold text-base text-left transition-all flex items-center gap-3 ${isAnswered && opt === q.answer ? "bg-green-500 text-white shadow-lg" : isAnswered && opt !== q.answer ? "bg-red-100 text-red-400" : "bg-white text-gray-700 hover:bg-red-50 border-2 border-gray-200 hover:border-red-300"}`}>
