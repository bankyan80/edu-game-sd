...
const subTopics = masterData.provinsi.map((item) => ({
  name: item.nama,
  detail: `${item.ibuKota} - ${item.pulau}`,
  emoji: "🏝️",
}));

const verifiedCount = masterData.provinsi.filter((item) => item.verifikasi).length;
const unverifiedCount = masterData.provinsi.filter((item) => !item.verifikasi).length;
