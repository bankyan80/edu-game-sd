...
export default function LaguNasionalPage() {
  ...
  const verifiedCount = masterData.laguNasional.filter((item) => item.verifikasi).length;
  const unverifiedCount = masterData.laguNasional.filter((item) => !item.verifikasi).length;
    ...
    <AppShell>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {masterData.laguNasional.map((item, i) => (
            <motion.div key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
              ...
              >
                ...
                <p className="text-sm text-gray-500 mb-1">{item.pencipta} ({item.tahun})</p>
